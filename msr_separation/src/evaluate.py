...
from src.utils import initialize_config, lightning_load_from_checkpoint, download_checkpoint_from_wandb
...
def evaluate_msr_bench(
    ...
    Args:
        ...
        precision: Precision setting for inference (default: "16-mixed")
        rank: Current process rank for multi-GPU evaluation (default: 0)
        world_size: Total number of processes for multi-GPU evaluation (default: 1)
    ...
    # Check if dataset is ordered by class (first 100 samples)
    if rank == 0:
        print("Checking dataset order...")
    sample_indices = list(range(num_samples))
    if shuffle_samples:
        # Use deterministic shuffling with fixed seed for reproducibility
        # All ranks use the same seed to get the same shuffled order, then shard
        shuffle_seed = 42
        rng = random.Random(shuffle_seed)

        # Check first 20 samples to see if they're all the same class (only rank 0)
        if rank == 0:
            first_labels = []
            ...
            if len(set(first_labels)) <= 2 and len(first_labels) >= 10:
                ...
                print(f"   Randomizing sample order to avoid bias...")
            else:
                print(f"✓ Dataset appears to be randomized or mixed")

        # Shuffle with deterministic seed (same on all ranks)
        rng.shuffle(sample_indices)
    else:
        if rank == 0:
            print(f"Using original dataset order (no shuffling)")

    # Shard sample indices across ranks to avoid processing duplicates
    # Each rank processes indices[rank::world_size]
    rank_indices = [sample_indices[i] for i in range(rank, len(sample_indices), world_size)]

    if rank == 0:
        print(f"Evaluating on {num_samples} total samples from MSRBench dataset (sharded across {world_size} GPUs)")
        print(f"  Rank 0 will process {len(rank_indices)} samples")
    else:
        print(f"[Rank {rank}] Processing {len(rank_indices)} samples (shard {rank+1}/{world_size})")

    print(f"[Rank {rank}] Using {window_duration}-second non-overlapping windows")
    print(f"[Rank {rank}] Model columns: {columns}")

    forward_pass_count = 0
    stats_interval = 500  # 10000

    # Use rank-specific progress bar description
    progress_desc = f"[Rank {rank}] Evaluating samples"
    if world_size > 1:
        progress_desc += f" ({rank+1}/{world_size})"

    with torch.no_grad():
        for sample_idx, idx in enumerate(tqdm(rank_indices, desc=progress_desc, disable=(rank != 0))):
            try:
                sample = dataset[idx]
    ...
    total_windows_processed = len(all_si_snr_values)
    print(f"[Rank {rank}] Finished processing: {total_windows_processed} windows processed, {sum(files_evaluated.values())} files evaluated")

    # Wait for all ranks to finish processing before gathering results
    if world_size > 1 and torch.distributed.is_available() and torch.distributed.is_initialized():
        print(f"[Rank {rank}] Waiting for all {world_size} ranks to complete processing...")
        torch.distributed.barrier()
        print(f"[Rank {rank}] All ranks finished. Gathering results from all {world_size} ranks...")

    # Gather results from all ranks if using multi-GPU
    if world_size > 1 and torch.distributed.is_available() and torch.distributed.is_initialized():

        # Prepare data to gather (convert lists to tensors for efficient gathering)
        # We'll gather the raw metric values and file counts

        # Gather SI-SNR values per source
        # Handle variable-length tensors by gathering sizes first, then padding
        gathered_si_snr_values = {}
        gathered_all_si_snr_values = []
        for label in columns:
            # Convert to tensor (empty tensor if no data)
            local_values = si_snr_values[label] if len(si_snr_values[label]) > 0 else []
            local_size = len(local_values)
            ...
            torch.distributed.all_gather(gathered_sizes, size_tensor)
            max_size = max([s.item() for s in gathered_sizes])
            
            # Create tensor (pad to max_size if needed)
            if local_size > 0:
                label_tensor = torch.tensor(local_values, dtype=torch.float32, device=device)
            else:
                label_tensor = torch.zeros((0,), dtype=torch.float32, device=device)
            
            # Pad to max_size for gathering
            if label_tensor.shape[0] < max_size:
                padding = torch.zeros((max_size - label_tensor.shape[0],), dtype=torch.float32, device=device)
                label_tensor = torch.cat([label_tensor, padding])
            
            # Now all ranks have tensors of the same size - gather
            gathered_tensors = [torch.zeros_like(label_tensor) for _ in range(world_size)]
            torch.distributed.all_gather(gathered_tensors, label_tensor)
            
            # Extract valid values (trim padding) and flatten
            valid_values = []
            for i, (tensor, size) in enumerate(zip(gathered_tensors, gathered_sizes)):
                actual_size = size.item()
                if actual_size > 0:
                    valid_values.extend(tensor[:actual_size].cpu().tolist())
            
            gathered_si_snr_values[label] = valid_values

        # Gather all SI-SNR values (handle variable sizes)
        local_size = len(all_si_snr_values)
        size_tensor = torch.tensor([local_size], dtype=torch.long, device=device)
        gathered_sizes = [torch.zeros_like(size_tensor) for _ in range(world_size)]
        torch.distributed.all_gather(gathered_sizes, size_tensor)
        max_size = max([s.item() for s in gathered_sizes])
        ...
        if local_size > 0:
            all_tensor = torch.tensor(all_si_snr_values, dtype=torch.float32, device=device)
        else:
            all_tensor = torch.zeros((0,), dtype=torch.float32, device=device)
        
        if all_tensor.shape[0] < max_size:
            padding = torch.zeros((max_size - all_tensor.shape[0],), dtype=torch.float32, device=device)
            all_tensor = torch.cat([all_tensor, padding])
        
        gathered_tensors = [torch.zeros_like(all_tensor) for _ in range(world_size)]
        torch.distributed.all_gather(gathered_tensors, all_tensor)
        ...
        for tensor, size in zip(gathered_tensors, gathered_sizes):
            ...
            if actual_size > 0:
                valid_values.extend(tensor[:actual_size].cpu().tolist())
        gathered_all_si_snr_values = valid_values

        # Gather Multi-Mel-SNR values per source (handle variable sizes)
        gathered_multi_mel_snr_values = {}
        gathered_all_multi_mel_snr_values = []
        for label in columns:
            local_values = multi_mel_snr_values[label] if len(multi_mel_snr_values[label]) > 0 else []
            local_size = len(local_values)
            ...
            gathered_multi_mel_snr_values[label] = valid_values

        # Gather all Multi-Mel-SNR values (handle variable sizes)
        local_size = len(all_multi_mel_snr_values)
        size_tensor = torch.tensor([local_size], dtype=torch.long, device=device)
        gathered_sizes = [torch.zeros_like(size_tensor) for _ in range(world_size)]
        torch.distributed.all_gather(gathered_sizes, size_tensor)
        max_size = max([s.item() for s in gathered_sizes])
        ...
        if local_size > 0:
            all_tensor = torch.tensor(all_multi_mel_snr_values, dtype=torch.float32, device=device)
        else:
            all_tensor = torch.zeros((0,), dtype=torch.float32, device=device)
        ...
        for tensor, size in zip(gathered_tensors, gathered_sizes):
            ...
            if actual_size > 0:
                valid_values.extend(tensor[:actual_size].cpu().tolist())
        gathered_all_multi_mel_snr_values = valid_values

        # Gather DT group statistics
        gathered_si_snr_by_dt_group = {group: [] for group in dt_group_names.keys()}
        gathered_multi_mel_snr_by_dt_group = {group: [] for group in dt_group_names.keys()}
        gathered_si_snr_by_dt_group_and_stem = {group: {label: [] for label in columns} for group in dt_group_names.keys()}
        gathered_multi_mel_snr_by_dt_group_and_stem = {group: {label: [] for label in columns} for group in dt_group_names.keys()}

        for dt_group in dt_group_names.keys():
            # Gather SI-SNR by DT group (handle variable sizes)
            local_values = si_snr_by_dt_group[dt_group] if len(si_snr_by_dt_group[dt_group]) > 0 else []
            local_size = len(local_values)
            size_tensor = torch.tensor([local_size], dtype=torch.long, device=device)
            gathered_sizes = [torch.zeros_like(size_tensor) for _ in range(world_size)]
            torch.distributed.all_gather(gathered_sizes, size_tensor)
            ...
            if local_size > 0:
                dt_tensor = torch.tensor(local_values, dtype=torch.float32, device=device)
            else:
                dt_tensor = torch.zeros((0,), dtype=torch.float32, device=device)
            
            if dt_tensor.shape[0] < max_size:
                padding = torch.zeros((max_size - dt_tensor.shape[0],), dtype=torch.float32, device=device)
                dt_tensor = torch.cat([dt_tensor, padding])
            
            gathered_tensors = [torch.zeros_like(dt_tensor) for _ in range(world_size)]
            torch.distributed.all_gather(gathered_tensors, dt_tensor)
            ...
            for tensor, size in zip(gathered_tensors, gathered_sizes):
                ...
                if actual_size > 0:
                    valid_values.extend(tensor[:actual_size].cpu().tolist())
            gathered_si_snr_by_dt_group[dt_group] = valid_values

            # Gather Multi-Mel-SNR by DT group (handle variable sizes)
            local_values = multi_mel_snr_by_dt_group[dt_group] if len(multi_mel_snr_by_dt_group[dt_group]) > 0 else []
            local_size = len(local_values)
            size_tensor = torch.tensor([local_size], dtype=torch.long, device=device)
            gathered_sizes = [torch.zeros_like(size_tensor) for _ in range(world_size)]
            torch.distributed.all_gather(gathered_sizes, size_tensor)
            ...
            for tensor, size in zip(gathered_tensors, gathered_sizes):
                ...
                if actual_size > 0:
                    valid_values.extend(tensor[:actual_size].cpu().tolist())
            gathered_multi_mel_snr_by_dt_group[dt_group] = valid_values

            for label in columns:
                # Gather SI-SNR by DT group AND stem (handle variable sizes)
                local_values = si_snr_by_dt_group_and_stem[dt_group][label] if len(si_snr_by_dt_group_and_stem[dt_group][label]) > 0 else []
                local_size = len(local_values)
                size_tensor = torch.tensor([local_size], dtype=torch.long, device=device)
                gathered_sizes = [torch.zeros_like(size_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_sizes, size_tensor)
                ...
                if local_size > 0:
                    stem_tensor = torch.tensor(local_values, dtype=torch.float32, device=device)
                else:
                    stem_tensor = torch.zeros((0,), dtype=torch.float32, device=device)
                
                if stem_tensor.shape[0] < max_size:
                    padding = torch.zeros((max_size - stem_tensor.shape[0],), dtype=torch.float32, device=device)
                    stem_tensor = torch.cat([stem_tensor, padding])
                
                gathered_tensors = [torch.zeros_like(stem_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_tensors, stem_tensor)
                ...
                for tensor, size in zip(gathered_tensors, gathered_sizes):
                    ...
                    if actual_size > 0:
                        valid_values.extend(tensor[:actual_size].cpu().tolist())
                gathered_si_snr_by_dt_group_and_stem[dt_group][label] = valid_values

                # Gather Multi-Mel-SNR by DT group AND stem (handle variable sizes)
                local_values = multi_mel_snr_by_dt_group_and_stem[dt_group][label] if len(multi_mel_snr_by_dt_group_and_stem[dt_group][label]) > 0 else []
                local_size = len(local_values)
                size_tensor = torch.tensor([local_size], dtype=torch.long, device=device)
                gathered_sizes = [torch.zeros_like(size_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_sizes, size_tensor)
                ...
                for tensor, size in zip(gathered_tensors, gathered_sizes):
                    ...
                    if actual_size > 0:
                        valid_values.extend(tensor[:actual_size].cpu().tolist())
                gathered_multi_mel_snr_by_dt_group_and_stem[dt_group][label] = valid_values

        # Gather file counts (use sum since each rank counts its own files)
        gathered_files_evaluated = {label: 0 for label in columns}
        for label in columns:
            count_tensor = torch.tensor([files_evaluated[label]], dtype=torch.int32, device=device)
            gathered_tensors = [torch.zeros_like(count_tensor) for _ in range(world_size)]
            torch.distributed.all_gather(gathered_tensors, count_tensor)
            gathered_files_evaluated[label] = int(torch.cat(gathered_tensors).sum().item())

        # Replace local results with gathered results
        si_snr_values = gathered_si_snr_values
        all_si_snr_values = gathered_all_si_snr_values
        multi_mel_snr_values = gathered_multi_mel_snr_values
        all_multi_mel_snr_values = gathered_all_multi_mel_snr_values
        si_snr_by_dt_group = gathered_si_snr_by_dt_group
        multi_mel_snr_by_dt_group = gathered_multi_mel_snr_by_dt_group
        si_snr_by_dt_group_and_stem = gathered_si_snr_by_dt_group_and_stem
        multi_mel_snr_by_dt_group_and_stem = gathered_multi_mel_snr_by_dt_group_and_stem
        files_evaluated = gathered_files_evaluated

        # For FAD-CLAP, we need to gather audio arrays from all ranks
        # This is more complex, so we'll do it separately
        if fad_clap_enabled:
            print(f"[Rank {rank}] Gathering FAD-CLAP audio data from all ranks...")
            # Note: FAD-CLAP gathering will be handled after initial results calculation
            # We'll gather the audio arrays and recalculate FAD-CLAP on rank 0

        print(f"[Rank {rank}] Results gathered successfully. Total samples processed across all ranks:")
        if rank == 0:
            total_windows = len(all_si_snr_values)
            print(f"  Total windows: {total_windows}")
            for label in columns:
                print(f"  {label}: {files_evaluated[label]} files, {len(si_snr_values[label])} windows")
    ...
    # For multi-GPU, gather FAD-CLAP audio data and calculate on rank 0
    if fad_clap_enabled:
        if world_size > 1 and torch.distributed.is_available() and torch.distributed.is_initialized():
            # Gather FAD-CLAP audio arrays from all ranks
            # This is complex because we need to gather variable-length lists of numpy arrays
            # We'll use a simpler approach: each rank calculates FAD-CLAP for its own data,
            # then we combine the embeddings before calculating the final FAD score

            # For now, we'll gather on rank 0 only (simpler but less efficient)
            # Rank 0 collects all audio arrays from other ranks
            if rank == 0:
                print("\n[Rank 0] Gathering FAD-CLAP audio data from all ranks...")
                # Collect from other ranks
                for other_rank in range(1, world_size):
                    # Receive data from other rank
                    # Note: This requires custom communication since we're sending lists of numpy arrays
                    # For simplicity, we'll calculate FAD-CLAP per rank and combine embeddings
                    pass

            # Alternative: Calculate FAD-CLAP embeddings per rank, then combine
            # This is more efficient
            print(f"\n[Rank {rank}] Calculating FAD-CLAP embeddings for this rank's data...")
            rank_fad_clap_targets_embeddings = {}
            rank_fad_clap_predictions_embeddings = {}

            for label in columns:
                if label in fad_clap_targets and len(fad_clap_targets[label]) > 0:
                    try:
                        target_emb = get_clap_embeddings_from_arrays(
                            fad_clap_targets[label], clap_model, clap_processor, clap_device, fad_clap_batch_size
                        )
                        pred_emb = get_clap_embeddings_from_arrays(
                            fad_clap_predictions[label], clap_model, clap_processor, clap_device, fad_clap_batch_size
                        )
                        if target_emb.size > 0 and pred_emb.size > 0:
                            rank_fad_clap_targets_embeddings[label] = target_emb
                            rank_fad_clap_predictions_embeddings[label] = pred_emb
                    except Exception as e:
                        print(f"[Rank {rank}] Warning: Failed to calculate FAD-CLAP embeddings for {label}: {e}")

            # Gather embeddings from all ranks (as numpy arrays converted to tensors)
            # This is more efficient than gathering raw audio
            gathered_fad_clap_targets_embeddings = {}
            gathered_fad_clap_predictions_embeddings = {}

            for label in columns:
                # Get embeddings for this label (or empty if not present)
                if label in rank_fad_clap_targets_embeddings:
                    target_emb = rank_fad_clap_targets_embeddings[label]
                    target_tensor = torch.from_numpy(target_emb).to(device)
                    embedding_dim = target_tensor.shape[1] if len(target_tensor.shape) > 1 else target_tensor.shape[0]
                else:
                    # Need to get embedding dim from another rank or use default
                    # First, try to get it from any rank that has embeddings
                    embedding_dim = 512  # Default CLAP embedding dimension
                    # We'll get the actual dim from the first rank that has data during gathering
                    target_tensor = torch.zeros((0, embedding_dim), dtype=torch.float32, device=device)

                # Gather sizes and embedding dimensions first to handle variable lengths
                sizes_tensor = torch.tensor([target_tensor.shape[0]], dtype=torch.long, device=device)
                dims_tensor = torch.tensor([target_tensor.shape[1] if len(target_tensor.shape) > 1 and target_tensor.shape[0] > 0 else embedding_dim],
                                          dtype=torch.long, device=device)
                gathered_sizes = [torch.zeros_like(sizes_tensor) for _ in range(world_size)]
                gathered_dims = [torch.zeros_like(dims_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_sizes, sizes_tensor)
                torch.distributed.all_gather(gathered_dims, dims_tensor)

                # Get max size and embedding dimension
                max_size = max([s.item() for s in gathered_sizes])
                actual_embedding_dim = max([d.item() for d in gathered_dims if d.item() > 0] or [embedding_dim])

                # Ensure tensor has correct shape
                if len(target_tensor.shape) == 1:
                    target_tensor = target_tensor.unsqueeze(0)
                if target_tensor.shape[1] != actual_embedding_dim:
                    # Reshape if needed
                    if target_tensor.shape[0] == 0:
                        target_tensor = torch.zeros((0, actual_embedding_dim), dtype=torch.float32, device=device)
                    else:
                        # This shouldn't happen, but handle it
                        target_tensor = target_tensor[:, :actual_embedding_dim] if target_tensor.shape[1] > actual_embedding_dim else target_tensor

                # Pad to max size if needed
                if target_tensor.shape[0] < max_size:
                    padding = torch.zeros((max_size - target_tensor.shape[0], actual_embedding_dim),
                                         dtype=target_tensor.dtype, device=device)
                    target_tensor = torch.cat([target_tensor, padding], dim=0)

                # Now gather (all tensors are same size)
                gathered_tensors = [torch.zeros_like(target_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_tensors, target_tensor)

                # Concatenate and trim to actual sizes, then convert to numpy
                valid_embeddings = []
                for i, (tensor, size) in enumerate(zip(gathered_tensors, gathered_sizes)):
                    actual_size = size.item()
                    if actual_size > 0:
                        valid_embeddings.append(tensor[:actual_size].cpu().numpy())

                if len(valid_embeddings) > 0:
                    combined = np.concatenate(valid_embeddings, axis=0)
                    gathered_fad_clap_targets_embeddings[label] = combined

                # Same for predictions
                if label in rank_fad_clap_predictions_embeddings:
                    pred_emb = rank_fad_clap_predictions_embeddings[label]
                    pred_tensor = torch.from_numpy(pred_emb).to(device)
                else:
                    pred_tensor = torch.zeros((0, actual_embedding_dim), dtype=torch.float32, device=device)

                sizes_tensor = torch.tensor([pred_tensor.shape[0]], dtype=torch.long, device=device)
                dims_tensor = torch.tensor([pred_tensor.shape[1] if len(pred_tensor.shape) > 1 and pred_tensor.shape[0] > 0 else actual_embedding_dim],
                                         dtype=torch.long, device=device)
                gathered_sizes = [torch.zeros_like(sizes_tensor) for _ in range(world_size)]
                gathered_dims = [torch.zeros_like(dims_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_sizes, sizes_tensor)
                ...
                max_size = max([s.item() for s in gathered_sizes])
                actual_embedding_dim_pred = max([d.item() for d in gathered_dims if d.item() > 0] or [actual_embedding_dim])

                if len(pred_tensor.shape) == 1:
                    pred_tensor = pred_tensor.unsqueeze(0)
                if pred_tensor.shape[1] != actual_embedding_dim_pred:
                    if pred_tensor.shape[0] == 0:
                        pred_tensor = torch.zeros((0, actual_embedding_dim_pred), dtype=torch.float32, device=device)
                    else:
                        pred_tensor = pred_tensor[:, :actual_embedding_dim_pred] if pred_tensor.shape[1] > actual_embedding_dim_pred else pred_tensor

                if pred_tensor.shape[0] < max_size:
                    padding = torch.zeros((max_size - pred_tensor.shape[0], actual_embedding_dim_pred),
                                         dtype=pred_tensor.dtype, device=device)
                    pred_tensor = torch.cat([pred_tensor, padding], dim=0)

                gathered_tensors = [torch.zeros_like(pred_tensor) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_tensors, pred_tensor)
                ...
                if len(valid_embeddings) > 0:
                    combined = np.concatenate(valid_embeddings, axis=0)
                    gathered_fad_clap_predictions_embeddings[label] = combined

            # Calculate FAD-CLAP scores from gathered embeddings (only on rank 0)
            if rank == 0:
                print("\n[Rank 0] Calculating final FAD-CLAP scores from gathered embeddings...")
                results['fad_clap'] = {'per_source': {}, 'overall': None}

                for label in sorted(columns):
                    if label in gathered_fad_clap_targets_embeddings and label in gathered_fad_clap_predictions_embeddings:
                        try:
                            target_emb = gathered_fad_clap_targets_embeddings[label]
                            pred_emb = gathered_fad_clap_predictions_embeddings[label]
                            if target_emb.size > 0 and pred_emb.size > 0:
                                fad_score = calculate_frechet_distance(target_emb, pred_emb)
                                if fad_score is not None:
                ...
                # Overall FAD-CLAP
                try:
                    all_targets = np.concatenate([gathered_fad_clap_targets_embeddings[label]
                                                 for label in columns
                                                 if label in gathered_fad_clap_targets_embeddings])
                    all_predictions = np.concatenate([gathered_fad_clap_predictions_embeddings[label]
                                                     for label in columns
                                                     if label in gathered_fad_clap_predictions_embeddings])
                    if all_targets.size > 0 and all_predictions.size > 0:
                        overall_fad = calculate_frechet_distance(all_targets, all_predictions)
                        if overall_fad is not None:
                            results['fad_clap']['overall'] = float(overall_fad)
                except Exception as e:
                    print(f"Warning: Failed to calculate overall FAD-CLAP: {e}")
            else:
                # Other ranks set empty FAD-CLAP results
                results['fad_clap'] = {'per_source': {}, 'overall': None}
        else:
            # Single GPU - calculate FAD-CLAP normally
...
def main():
    ...
    # Load model using standard Lightning checkpoint loading
    # Note: on_load_checkpoint hook in BaseLightningModule will automatically load EMA state if present
    lightning_module = lightning_load_from_checkpoint(config['lightning_module'], checkpoint_path)
    ...
    # The EMA state should have been loaded automatically via on_load_checkpoint hook
    ...
    try:
        ...
        if args.evaluate_msr_bench:
            ...
            results = evaluate_msr_bench(
                ...
                shuffle_samples=True,  # Randomize to avoid bias if dataset is ordered
